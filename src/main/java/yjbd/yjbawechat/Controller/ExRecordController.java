...
public class ExRecordController {
    ...
    public String UpdateIdRecord(HttpServletRequest request,exEnforceModel exEnforceModel){
        ...
        String checke_item_datail = request.getParameter("checke_item_datail");
        JSONObject jsonObject = JSONObject.parseObject(checke_item_datail);
        Map<String,Object> map=new HashMap<>(5);
        map.putAll(jsonObject);
        map.forEach((key,value)->{
            String[] split = key.split("\\：");
            expertService.updateExpertItemRecordById(value+"",record_id,split[1]);
        });
...
//        String ON_SCENE_RECORD=request.getParameter ( "ON_SCENE_RECORD" );
        punishMeasureService.setExPunishMeasureInfo(PROCESS_DECISION,CHECKE_DETAIL,CHECKED_UNIT,CHECKED_START_TIME,RECORD_ID);
    }
    /**
     * 缓存记录专家现场处理措施信息
     */
