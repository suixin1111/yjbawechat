...
public class RecordController {
    ...
    public String UpdateIdRecord(HttpServletRequest request, EnforceModel enforceModel){
        ...
        String checke_item_name = request.getParameter("CHECKE_ITEM_NAME");
        JSONObject jsonObject = JSONObject.parseObject(checke_item_name);
        Map<String,Object> map = new HashMap<>(5);
        map.putAll(jsonObject);
        map.forEach((key,val) -> {
            String str = key.replaceAll("[\\S]+\\：", "");
            noticeViewService.updateItemRecord(val+"",record_id,str);
        });
        String responseText="";
    ...
    public String createEventSubmit(HttpServletRequest request){
        ...
        enforceModel.setS_CODE(request.getParameter("S_CODE"));
