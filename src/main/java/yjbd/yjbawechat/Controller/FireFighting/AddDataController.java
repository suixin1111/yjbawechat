...
import yjbd.yjbawechat.Util.FirePdfUtil;
import yjbd.yjbawechat.Util.StreetMsg;
...
public class AddDataController {
    ...
    public Map<String,Object> insetCheckItem(HttpServletRequest request) throws IOException{
        ...
        fireIndexService.getCheckUnitTable(CHECK_UNIT);

        String ID = fireIndexService.getCheckUnitPro(CREATE_TIME);
        String fightingPdf = FirePdfUtil.createPdfNoSign("fightingPdf", ID, CHECKED_UNIT, CHECK_UNIT, CHECK_PEOPLE, CHECK_START_TIME, CHECK_ITEM, CHECK_DESC, CHECK_IMG,CHECK_UNIT);
        fireIndexService.updateCheckPdf(fightingPdf,"暂无",CREATE_TIME);
        fireIndexService.updateInter();
    ...
    public Map<String,Object> createPdf(HttpServletRequest request) throws IOException{
        ...
        fireIndexService.getCheckUnitTable(CHECK_UNIT);
        String ID1 = fireIndexService.getCheckUnitPro(CREATE_TIME);
        String pathAB = yjbd.yjbawechat.Util.CreatePdfUtil.createPdf("fightingPdf",ID1,CHECKED_UNIT,CHECK_UNIT,CHECK_PEOPLE,CHECK_START_TIME,CHECK_ITEM,CHECK_DESC,CHECKMAN_SIGN,CHECKEDMAN_SIGN,CHECK_IMG,CHECK_UNIT);
        String[] arr = pathAB.split("_");
        yjbd.yjbawechat.VO.ResultVO resultVO = fireIndexService.updateCheckPdf(arr[0],arr[1],CREATE_TIME);
        fireIndexService.updateInter();
